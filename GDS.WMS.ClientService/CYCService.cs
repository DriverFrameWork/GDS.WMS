    ...
    public class CYCService : IJob
    {
        ...
        public void Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                if (!Sftp.IsConnected)
                    Sftp.Connect();
                logger.Info("同步盘点事务主数据任务开始运行");
                var sw = new Stopwatch();
                sw.Start();
                var service = ServicesFactory.GetInstance<IStocking>();
                logger.Info("盘点开始");
                var pni = service.Run(Ssh, Sftp);
                logger.Info(pni.ErrorMessage + " " + pni.Count + " " + pni.IsSuccess);
                logger.Info("盘点结束");
                sw.Stop();
                logger.Info("同步盘点事务主数据任务结束运行,总运行时间:" + sw.Elapsed.TotalMilliseconds + "毫秒");
                Ssh.RunCommand("exit");
                Ssh.Disconnect();
                Sftp.Disconnect();
            }
            ...
            {
                logger.Error("同步盘点事务主数据任务运行异常", ex);
            }
        }
    }
}
